...
class network:
    ...
    def from_weights(weights, neurons_per_layer, activation_function):
        # create instance: (weights will be initialized to random values)
        obj = network(neurons_per_layer, activation_function)
        # sanity check:
    ...
    def __init__(self, neurons_per_layer, activation_function):
        ...
        self.allLayers = Layerwrapper(neurons_per_layer, activation_function)

        self.previous_weight_change = [np.zeros(l.weights.shape) for l in self.allLayers.layers[:-1]]
    ...
    def calculate_outputs(self, inputs):
        self.allLayers.layers[0].outputs = np.append(inputs, 1)  # the 1 is added as a bias value

        for i in range(self.num_layers - 1):
            self.allLayers.layers[i+1].inputs = self.allLayers.layers[i].calculate_output()

        return self.allLayers.layers[-1].calculate_output()
    ...
    def backpropagate(self, network_output, true_value):
        ...
        self.allLayers.layers[-1].delta_values = np.transpose(network_output - true_value)

        # Calculate delta values (error) of hidden layers
        for i in reversed(range(1, self.num_layers - 1)):
            # No deltas for the bias value nodes
            w_mod = self.allLayers.layers[i].weights[0:-1,:]
            self.allLayers.layers[i].delta_values = w_mod.dot(self.allLayers.layers[i + 1].delta_values) * self.allLayers.layers[i].derivatives
    ...
    def calc_update_weights(self, learning_rate):
        weight_changes = []
        for i in range(self.num_layers-1):
            weight_change = -learning_rate * np.outer(self.allLayers.layers[i+1].delta_values, self.allLayers.layers[i].outputs).T
    ...
    def update_weights(self, weight_changes, use_momentum=False, beta=None):
        if use_momentum:
            for i in range(self.num_layers-1):
                self.allLayers.layers[i].weights += (weight_changes[i] + beta * self.previous_weight_change[i])
        else:
            for i in range(self.num_layers-1):
                self.allLayers.layers[i].weights += weight_changes[i]
    ...
    def train_batch(self, training_data: Sequence[trial_run], learning_rate, use_momentum=False, beta=None):
        ...
        running_total = [np.zeros(l.weights.shape) for l in self.allLayers.layers[:-1]]
